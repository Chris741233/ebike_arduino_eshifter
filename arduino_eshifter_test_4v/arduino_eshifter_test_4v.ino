...
*************** 15.09.2026 -- MODIFICATION POUR version 4 VITESSES !! **********************
...
const int TIMEOUT_1to4 = 3600;  // ms, temps de reference pour passer de v1 à v4 (butee basse a haute)
const int MARGE_HOME = 100;     // ms, petite marge a ajouter a TIMEOUT_1to4 pour etre sur de la position home v1 au boot
const int TIMEOUT_STEP = TIMEOUT_1to4/3; // ms, etape (1to2, 2to3, etc..), en principe TIMEOUT_1to4 / 3
...
void loop()
{
    ...
    if(state_shifter==0) mstartmotor = (TIMEOUT_1to4+MARGE_HOME)-SUP_PIC;
    ...
    if(state_shifter < 1 && !motor_turn) {
        ...
        timer_motor.start(TIMEOUT_1to4 + MARGE_HOME );  // trajet au plus long + petite marge
